...
public class ValidateHearingQueryRamlConfigAndPassThroughRequesterTest {
    ...
    public void setup() throws IOException {
        for (Method method : HearingQueryView.class.getMethods()) {
         ...
         */
        List<String> allLines = FileUtils.readLines(new File("src/raml/hearing-query-view.raml"));
    ...
    public void testActionNamesAreHandledProperly() throws Exception {
        for (Map.Entry<String, String> entry : apiHandlers.entrySet())
            assertThat(HearingQueryView.class, isHandlerClass(Component.QUERY_VIEW).with(method(entry.getKey()).thatHandles(entry.getValue())));
    }
