...
import static uk.gov.moj.cpp.hearing.event.nows.activiti.worlflow.materialupload.listener.MaterialFileUploadedListener.MATERIAL_EVENTS_FILE_UPLOADED;
...
import uk.gov.moj.cpp.hearing.event.nows.activiti.worlflow.materialupload.listener.MaterialFileUploadedListener;


@RunWith(MockitoJUnitRunner.class)
public class MaterialFileUploadedListenerTest {

    private static final String USER_ID = randomUUID().toString();
    @Mock
    private ActivitiService activitiService;
    @InjectMocks
    private MaterialFileUploadedListener materialFileUploadedListener;
    @Mock
    ...
    public void shouldSignalProcessWithActivitiIdIfCaseIdPresent() {
        ...
        when(activitiService.signalProcessByActivitiIdAndFieldName(eq(RECEIVE_MATERIAL_UPLOADED_CONFIRMATION), eq(MATERIAL_ID), eq(materialId.toString()))).thenReturn(true);

        //when
        this.materialFileUploadedListener.processEvent(event);
        //then
        verify(activitiService).signalProcessByActivitiIdAndFieldName(eq(RECEIVE_MATERIAL_UPLOADED_CONFIRMATION), eq(MATERIAL_ID), eq(materialId.toString()));
    }
    ...
    public void shouldNotSignalProcessWithActivitiIdIfCaseIdNotPresent() {
        ...
        when(event.metadata()).thenReturn(getMetedata());
        final JsonEnvelope jsonEnvelope = assembleEnvelopeWithPayloadAndMetaDetails(payload, MATERIAL_EVENTS_FILE_UPLOADED, fakeId.toString(), USER_ID.toString());

        //when
        materialFileUploadedListener.processEvent(jsonEnvelope);
        //then
