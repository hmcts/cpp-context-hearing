...
public class PaymentTermsCalculator {
    ...
    public String calculatePaymentTerms(final Map<UUID, Prompt> id2PromptRef, final List<ResultLine> resultLines4Now, final Boolean isWelsh) {
        ...
        final boolean hasOptionalText = nonNull(getPromptValue(id2PromptRef, resultLines4Now, P_DEFAULT_DAYS_IN_JAIL_PROMPT_REFERENCE));
        ...
        if (hasPayDateResult) {
            if (hasOptionalText) {
                return String.format(PAY_BY_DATE + OPTIONAL_TEXT, LocalDate.parse(getPromptValue(id2PromptRef, resultLines4Now, P_PAY_BY_DATE)).format(FORMATTER),
                        getPromptValue(id2PromptRef, resultLines4Now, P_DEFAULT_DAYS_IN_JAIL_PROMPT_REFERENCE));
            } else {
    ...
    private String getPromptValue(final Map<UUID, Prompt> id2PromptRef, final List<ResultLine> resultLines4Now, final String promptReference) {
        ...
        if (hasPayDateResult) {
            if (hasOptionalText) {
                return String.format(PAY_BY_DATE_WELSH + OPTIONAL_TEXT_WELSH, LocalDate.parse(getPromptValueWelsh(id2PromptRef, resultLines4Now, P_PAY_BY_DATE)).format(FORMATTER_WELSH),
                        getPromptValueWelsh(id2PromptRef, resultLines4Now, P_DEFAULT_DAYS_IN_JAIL_PROMPT_REFERENCE));
            } else {
    ...
    private String getPromptValueWelsh(final Map<UUID, Prompt> id2PromptRef, final List<ResultLine> resultLines4Now, final String promptReference) {
        return resultLines4Now.stream().flatMap(rl -> rl.getPrompts().stream())
                       .filter(p -> promptReference.equals(id2PromptRef.get(p.getId()).getReference())
                       )
