...
public class HearingEventProcessor {
    ...
    public void publicDraftResultSavedV2PublicEvent(final JsonEnvelope event) {
        if(LOGGER.isErrorEnabled())  {LOGGER.error("recieved private  event hearing.draft-result-saved-v2 correlationId: {}" , event.metadata().clientCorrelationId().orElse(null));}
        if (LOGGER.isDebugEnabled()) {
        ...
        if(LOGGER.isErrorEnabled())  {LOGGER.error("first conversion in event hearing.draft-result-saved-v2 correlationId: {}" , event.metadata().clientCorrelationId().orElse(null));}
