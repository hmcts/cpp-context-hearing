...
public class HearingEventProcessor {
    ...
    public void publicDraftResultSavedV2PublicEvent(final JsonEnvelope event) {
        if (LOGGER.isErrorEnabled()) {
            LOGGER.error("INV: privateeventsavedraft2  clienCorrelationId: {}", event.metadata().clientCorrelationId().orElse(null));
        }

        if (LOGGER.isWarnEnabled()) {
            LOGGER.warn("recieved private  event hearing.draft-result-saved-v2 correlationId: {}", event.metadata().clientCorrelationId().orElse(null));
        }
        if (LOGGER.isDebugEnabled()) {
        ...
        final JsonObject publicEventPayload = this.objectToJsonObjectConverter.convert(publicHearingDraftResultSaved);
        if (LOGGER.isWarnEnabled()) {
            LOGGER.warn("raising public event hearing.draft-result-saved-v2 correlationId: {}", event.metadata().clientCorrelationId().orElse(null));
        }
        this.sender.send(envelopeFrom(metadataFrom(event.metadata()).withName(PUBLIC_HEARING_DRAFT_RESULT_SAVED), publicEventPayload));
        if (LOGGER.isWarnEnabled()) {
