...
public class HearingEventStreamTransformTest {
    ...
    public void shouldTransformHearingInitiatedEvent() {
        ...
        when(eventInstance.transform(Mockito.anyObject(), Mockito.anyObject())).thenReturn(transformedPayload);

        final JsonEnvelope expected = underTest.apply(event).findFirst().get();

        verify(eventInstance).transform(Mockito.anyObject(), Mockito.anyObject());
