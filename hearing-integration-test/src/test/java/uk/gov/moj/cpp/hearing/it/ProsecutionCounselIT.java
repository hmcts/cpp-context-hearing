...
public class ProsecutionCounselIT extends AbstractIT {
    ...
    @Test
    public void testProsecutionCounselOperations_shouldAddUpdateAndRemove() {

        final InitiateHearingCommandHelper hearingOne = h(UseCases.initiateHearing(getRequestSpec(), standardInitiateHearingTemplate()));

        ProsecutionCounsel firstProsecutionCounsel;
        try (final EventListener publicProsecutionCounselAdded = listenFor("public.hearing.prosecution-counsel-added")
                .withFilter(isJson(withJsonPath("$.hearingId", is(hearingOne.getHearingId().toString()))))) {

            firstProsecutionCounsel = createFirstProsecutionCounsel(hearingOne);
        ...
        final EventListener publicProsecutionCounselAdded = listenFor("public.hearing.prosecution-counsel-change-ignored")
                .withFilter(isJson(withJsonPath("$.hearingId", is(hearingOne.getHearingId().toString()))));

        addProsecutionCounsel(getRequestSpec(), hearingOne.getHearingId(),
        ...
        // Update prosecution counsel
        final EventListener publicProsecutionCounselUpdated = listenFor("public.hearing.prosecution-counsel-updated")
                .withFilter(isJson(withJsonPath("$.hearingId", is(hearingOne.getHearingId().toString()))));
        ...
        // Remove prosecution counsel
        final EventListener publicProsecutionCounselRemoved = listenFor("public.hearing.prosecution-counsel-removed")
                .withFilter(isJson(withJsonPath("$.hearingId", is(hearingOne.getHearingId().toString()))));

        removeProsecutionCounsel(getRequestSpec(), hearingOne.getHearingId(),
