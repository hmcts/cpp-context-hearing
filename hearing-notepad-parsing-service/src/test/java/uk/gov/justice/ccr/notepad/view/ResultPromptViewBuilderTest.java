...
public class ResultPromptViewBuilderTest {
    ...
    public void buildFromKnowledgeWithOneOf() throws Exception {
        ...
        assertThat(Arrays.asList(p1.getCode().length(), p1.getLabel(), p1.getType(), p1.getRequired())
                , containsInAnyOrder(Arrays.asList(36, "Treatment institution / place", TXT, Boolean.TRUE).toArray()));

        assertThat(Arrays.asList(p2.getCode().length(), p2.getLabel(), p2.getType(), p2.getRequired())
                , containsInAnyOrder(Arrays.asList(36, "Under direction of", TXT, Boolean.TRUE).toArray()));

        assertThat(Arrays.asList(p3.getCode().length(), p3.getLabel(), p3.getType(), p3.getRequired())
                , containsInAnyOrder(Arrays.asList(36, "Period of treatment", DURATION, Boolean.TRUE).toArray()));

        assertThat(Arrays.asList(p4.getCode().length(), p4.getLabel(), p4.getComponentType(), p4.getRequired())
                , containsInAnyOrder(Arrays.asList(36, "Period of treatment-oneOf", "ONEOF", Boolean.TRUE).toArray()));

        assertThat(Arrays.asList(p5.getCode().length(), p5.getLabel(), p5.getType(), p5.getRequired())
                , containsInAnyOrder(Arrays.asList(36, "Treatment intervals", TXT, Boolean.FALSE).toArray()));
