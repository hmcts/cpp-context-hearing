...
public abstract class OffenceRepository extends AbstractEntityRepository<Offence, HearingSnapshotKey> {
    ...
    @Query(value = "from Offence o where o.id.id = :offenceId and o.originHearingId = :originHearingId")
    public abstract List<Offence> findByOffenceIdOriginHearingId(@QueryParam("offenceId") final UUID offenceId, @QueryParam("originHearingId") final UUID originHearingId);
}