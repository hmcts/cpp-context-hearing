...
| LPT-2506 | Schedule job at 1AM to purge expired unconfirmed bookings | READY FOR TEST | **Already merged — nothing to do** | The scheduling patch is already merged upstream as `patches/feature-LPT-2506-purge-all-expired-unconfirmed-bookings`. Same baseUri, resource, media type and `0 0 1 * * ?` cron as the one drafted during this work, which was therefore deleted as a duplicate — applying both would have registered two jobs hitting the same endpoint at 01:00. It was blocked on LPT-2528 (the purge that did not restore capacity); with that fixed it is ready for retest. |
